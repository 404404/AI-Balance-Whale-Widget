...
3. 从本 fork 的 [macos-v0.1.0-beta.7 Release](https://github.com/404404/AI-Balance-Whale-Widget/releases) 下载 DMG，将 `AI Balance Whale.app` 拖入 `Applications` 后启动。首次预发布为 ad-hoc 签名，未经过 Apple Developer ID 签名/公证；若 macOS 阻止打开，请在「系统设置 → 隐私与安全性」确认后选择仍要打开。不要关闭 Gatekeeper。
...
APP_VERSION=0.1.0-beta.7 BUILD_NUMBER=2 MacOSApp/scripts/build.sh
APP_VERSION=0.1.0-beta.7 MacOSApp/scripts/create-dmg.sh
```
...
应用帮助、反馈和发布链接使用本 fork；小鲸鱼素材、DSH 插件代码及原作者归属仍保留，感谢 [MeteorNOX/DeepSeek-Balance-Whale-Widget](https://github.com/MeteorNOX/DeepSeek-Balance-Whale-Widget)。
