...
## Beta 8 verification boundary

The beta.8 candidate mounts the upstream editor directly in the unified settings page, routes native input through `NSPanel.sendEvent`, and moves app-server framing/request state off the UI queue. CI runs Swift fixtures, packaged WKWebView geometry/input/editor tests, arm64 bundle checks, DMG mount checks and ad-hoc codesign integrity checks. This Linux execution environment cannot run Finder, AppKit or a real Codex account; GUI/pointer/Spaces and real-account checks remain explicitly unverified until a user runs the uploaded DMG on macOS. CI fixtures never contain credentials.
