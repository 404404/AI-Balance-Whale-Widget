...
| Codex account and rate-limit windows | App-owned browser OAuth (PKCE + loopback callback), Keychain credential store and native WHAM URLSession client | Implemented without Codex CLI, app-server or `CODEX_HOME`: browser login returns a credential to the native coordinator, tokens remain in the App-owned Keychain, and WHAM primary/secondary/additional windows are parsed with account-scoped request generations. Login, quota-readable, stale-cache and permission failures remain distinct. |
...
| Audio settings | Direct upstream audio-group/resource panels plus App controls | Implemented; built-in/imported groups, press/release slots, preview and crop management are reachable from the unified settings page and persist without restarting the quota client. |
...
## Beta 13 verification boundary

The beta.13 candidate mounts the upstream editor directly in the unified settings page, routes native input through `NSPanel.sendEvent`, preserves the canonical upstream bubble fixture, and carries a per-refresh account/request identity through the native WHAM client. CI runs source fixtures, packaged WKWebView geometry/input/editor/OAuth-bridge tests, non-empty JUnit output checks, arm64 bundle checks, DMG mount checks and ad-hoc codesign integrity checks. This Linux execution environment cannot run Finder, AppKit or a real ChatGPT account; GUI/pointer/Spaces and real-account checks remain explicitly unverified until the uploaded DMG is run on macOS. CI fixtures never contain credentials.
