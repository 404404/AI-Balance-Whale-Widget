...
var dshwEnabled = !!window.__AIWhaleStandalone
if (!dshwEnabled) {
  try {
    ...
    if (dshwIsChatRoot(dshwRoot)) {
      ...
      var dshwPoll = setInterval(function () {
        ...
        if (dshwPollTries >= 10) clearInterval(dshwPoll)
      }, 500)
    }
  } catch (err) {}
}
...
var IMG_URL = window.__AIWhaleStandalone ? 'DSniang1.png' : '/dsh-whale/image.png?v=2'
var GIF_URL = window.__AIWhaleStandalone ? 'rua.gif' : '/dsh-whale/rua.gif'
var BUBBLE_URL = '/dsh-whale/bubble.json'

function dshwResourceURL(kind, id) {
  if (!window.__AIWhaleStandalone) return null
  try {
    var map = window.__AIWhaleResourceURLs && window.__AIWhaleResourceURLs[kind]
    if (map && map[id]) return map[id]
  } catch (err) {}
  if (kind === "bubbles" && id === "bimg_petpet") return "bubble-petpet.gif"
  if (kind === "bubbles" && (id === "bimg_money" || id === "bimg_money1")) return "bubble-money1.gif"
  if (kind === "roles" && id === "default") return "DSniang1.png"
  if (kind === "audio" && id === "ya1") return "Ya1.mp3"
  if (kind === "audio" && id === "ya2") return "Ya2.mp3"
  return null
}
function dshwBubbleImageURL(id) {
  return dshwResourceURL("bubbles", id) || "/dsh-whale/bubble-img.png?id=" + encodeURIComponent(id)
}
function dshwRoleImageURL(id) {
  return dshwResourceURL("roles", id) || "/dsh-whale/role-image.png?id=" + encodeURIComponent(id)
}
function dshwSoundURL(slot, group) {
  var key = group || slot
  return dshwResourceURL("audio", "" + slot + ":" + key) || dshwResourceURL("audio", key) || "/dsh-whale/sound/" + slot + ".mp3?set=" + encodeURIComponent(group || "")
}
function dshwAudioFragmentURL(id) {
  return dshwResourceURL("audio", id) || "/dsh-whale/audio-fragment.wav?id=" + encodeURIComponent(id)
}
...
try {
  ...
  if (initRoleId && initRoleId !== 'default') initRoleUrl = dshwRoleImageURL(initRoleId)
} catch (err) {}
img.src = initRoleUrl
img.addEventListener("error", function () {
  if (window.__AIWhaleStandalone && img.getAttribute("src") !== IMG_URL) {
    img.src = IMG_URL
    try { window.__AIWhaleNativePost && window.__AIWhaleNativePost("imageFallback", { source: "builtin" }) } catch (err) {}
  }
})
img.alt = 'DeepSeek 余额'
img.draggable = false
...
function playTaskEndSound() {
  try {
    ...
    if (sel.indexOf('frag:') === 0) url = dshwAudioFragmentURL(sel.slice(5))
...
function playTaskEndGroupClick(groupId) {
  try {
    ...
    if (pressEmpty) {
      if (!releaseEmpty) {
        var relOnly = new Audio(dshwSoundURL('release', groupId))
    ...
    var press = new Audio(dshwSoundURL('press', groupId))
    ...
    var release = new Audio(dshwSoundURL('release', groupId))
...
function resPlayFragment(fid) {
  try {
    ...
    var a = new Audio(dshwAudioFragmentURL(fid))
    try { a.volume = Number(soundVol) || 0.9 } catch (err) {}
...
function resRenderData(wrap, roles, bubbleImgs, audio) {
  try {
    ...
    bubbleImgs.forEach(function (im) {
      ...
      wrap.appendChild(resImgRow(dshwBubbleImageURL(im.id), im.name || im.id, '', [tag, resMkDel('删除', !!im.builtin, function () { resDelBubbleImg(im.id) })]))
    })
    if (!anyImg) {
      var empty = document.createElement('div')
...
function usageAppendLine(body, m, below, amount) {
  try {
    ...
    if (bubbleIsImgMod(m)) {
      ...
      imEl2.src = dshwBubbleImageURL(imgId2)
...
function bubbleChoiceWeight(o) { var n = Number(o && o.w); return isFinite(n) && n >= 0 ? Math.round(n) : 1 }
...
function openQuickModuleEditor(m, anchorBtn) {
  ...
  if (m.type === 'plan' && (apiPlanMultiWin(m.modelId) || apiPlanWinList(m.modelId))) {
    ...
    var winOpts = bubblePlanWinOptions(m.modelId)
    for (var wi = 0; wi < winOpts.length; wi++) {
      var wo = document.createElement('option')
      wo.value = winOpts[wi][0]
      wo.textContent = winOpts[wi][1]
      wsel.appendChild(wo)
    }
...
function renderModuleEditor() {
  ...
  if (m.type === 'text') {
    ...
    moduleImgSelect.addEventListener('change', function () {
      ...
      if (m.imgId) { moduleImgPreviewEl.src = dshwBubbleImageURL(m.imgId); moduleImgPreviewEl.style.display = 'block' }
        ...
        (function (idx2) {
          ...
          function syncTh2() {
            try {
              ...
              if (it2.imgId) th2.src = dshwBubbleImageURL(it2.imgId)
...
function bubblePlanWinOptions(modelId) {
  var out = [["all", "全部窗口"]]
  var list = apiPlanWinList(modelId)
  if (list && list.length) {
    for (var i = 0; i < list.length; i++) {
      var w = list[i] || {}
      if (w.key) out.push([String(w.key), String(w.label || w.key)])
    }
  } else {
    out = BUBBLE_PLAN_WIN_OPTS.slice()
  }
  return out
}
function bubblePlanWinOf(m) {
  ...
  var opts = bubblePlanWinOptions(m.modelId)
  for (var i = 0; i < opts.length; i++) { if (opts[i][0] === w) return w }
  return 'all'
}
function bubblePlanWinLabel(w, modelId) {
  var opts = bubblePlanWinOptions(modelId)
  for (var i = 0; i < opts.length; i++) { if (opts[i][0] === w) return opts[i][1] }
  return opts[0][1]
}
...
function bubblePlanModuleLabel(m) {
  ...
  return '额度·' + nm + (w === 'all' ? '' : '（' + bubblePlanWinLabel(w, m.modelId) + '）')
}
...
function bubblePickLine(lines, avoidIdx) {
  ...
  for (var i = 0; i < lines.length; i++) {
    var n = Number(lines[i] && lines[i].w)
    var weight = isFinite(n) && n >= 0 ? n : 1
    weights.push(weight)
    total += weight
  }
  if (!(total > 0)) { for (var zi = 0; zi < weights.length; zi++) weights[zi] = 1; total = weights.length }
  var pick
...
function bubbleRowsTo(parentEl, mods) {
  ...
  for (var g = 0; g < groups.length; g++) {
    ...
    if (bubbleIsImgMod(grp[0])) {
      ...
      im.src = dshwBubbleImageURL(pickId)
      im.alt = ''
      im.draggable = false
...
function applySoundSet() {
  try {
    ...
    if (pEmpty) { pressAudio = null } else {
      pressAudio = new Audio(dshwSoundURL('press', soundSet))
      pressAudio.preload = 'auto'
      pressAudio.volume = soundVol
    }
    if (rEmpty) { releaseAudio = null } else {
      releaseAudio = new Audio(dshwSoundURL('release', soundSet))
...
function audioEditPreviewEnsure(force) {
  try {
    ...
    if (pressId) {
      audioEditPreviewEl = new Audio(dshwAudioFragmentURL(pressId))
      audioEditPreviewEl.preload = 'auto'
      audioEditPreviewEl.volume = soundVol
    }
    if (releaseId) {
      audioEditPreviewRelease = new Audio(dshwAudioFragmentURL(releaseId))
...
window.addEventListener("aiwhale-data", function () { try { loadApiModels(null, true) } catch (err) {} })
